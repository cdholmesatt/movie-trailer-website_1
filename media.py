...
class Movie():
        ...
        def __init__(self, movie_title):
                ...
                # Function definition - Search youtube for movie_title
                # passed from media.py, return trailer_youtube value

                def search_youtube():
                    search_text = self.title+" trailer official"
                    movie = {"search_query": search_text}
                    search = urllib.urlopen(
                        youtube+'results'+"?"+urllib.urlencode(movie))
                    result = search.read()
                    search.close()
                    search_exp = '\"yt-lockup-title\"\>'
                    search_exp2 = '\<a href=\"\/watch\?v\=([\w|\d|-|_]+)\"\s'
                    match = re.search(search_exp+search_exp2, result)
                    if match:
                        match.group(1)
                        trailer_youtube = youtube+'watch?v='+match.group(1)
                        return trailer_youtube

                # Function definition - Searches omdb for the movie_title
                # passed from media.py, returns elements associated with the
                # contents of movie_title via api
                def search_omdb():
                    connection = urllib.urlopen(
                        omdb+self.title+"&y=&plot=short&r=json")
                    jsoninput = connection.read()
                    decoded = json.loads(jsoninput)
                    return decoded

                # Function definition - Uses the movie_title passed from
                # media.py and the imdbID passed from search_omdb() to build
                # the associated imdb url
                def search_imdb(decoded):
                        imdbURL = imdb+decoded['imdbID']+"/?ref_=nv_sr_1"
                        # Plot, Poster, trailer_youtube, Released,
                        # Year, Genre, Director, Rated, Actors):
                        return imdbURL

                # Function definition - Uses the dictionary passed
                # from search_omdb api
